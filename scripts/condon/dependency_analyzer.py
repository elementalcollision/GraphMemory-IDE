"""
Condon Dependency Analyzer for Compilation Planning

This module provides comprehensive dependency analysis for Condon compilation planning.
...
- Condon-specific compilation optimization
"""
...
class ModuleStatus(Enum):
    """Module compilation status for Condon"""

    CONDON_COMPATIBLE = "condon_compatible"
    CONDON_OPTIMIZABLE = "condon_optimizable"
    CONDON_LIMITED = "condon_limited"
    CONDON_INCOMPATIBLE = "condon_incompatible"
...
class ModuleInfo:
    ...
    condon_status: ModuleStatus = ModuleStatus.CONDON_COMPATIBLE
...
class DependencyGraph:
    ...
    thread_safety_issues: List[str] = field(default_factory=list)
    condon_optimization_opportunities: List[str] = field(default_factory=list)


class CondonDependencyAnalyzer:
    """Comprehensive dependency analyzer for Condon compilation planning"""
    ...
    def analyze_codebase(self) -> DependencyGraph:
        ...
        # Assess Condon compatibility
        self._assess_condon_compatibility()
    ...
    def _create_compilation_order(self) -> None:
        """Create optimal compilation order for Condon"""
        try:
    ...
    def _assess_condon_compatibility(self) -> None:
        """Assess Condon compatibility for each module"""
        for module_name, module_info in self.graph.modules.items():
            # Check for Condon-incompatible patterns
            incompatibilities = []
            ...
            # Determine Condon status
            if incompatibilities:
                if len(incompatibilities) > 3:
                    module_info.condon_status = ModuleStatus.CONDON_INCOMPATIBLE
                elif len(incompatibilities) > 1:
                    module_info.condon_status = ModuleStatus.CONDON_LIMITED
                else:
                    module_info.condon_status = ModuleStatus.CONDON_OPTIMIZABLE
            else:
                module_info.condon_status = ModuleStatus.CONDON_COMPATIBLE
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "condon_compatible": len(
                    [
                        m
                        for m in self.graph.modules.values()
                        if m.condon_status == ModuleStatus.CONDON_COMPATIBLE
                    ]
                ),
                "condon_optimizable": len(
                    [
                        m
                        for m in self.graph.modules.values()
                        if m.condon_status == ModuleStatus.CONDON_OPTIMIZABLE
                    ]
                ),
                "condon_limited": len(
                    [
                        m
                        for m in self.graph.modules.values()
                        if m.condon_status == ModuleStatus.CONDON_LIMITED
                    ]
                ),
                "condon_incompatible": len(
                    [
                        m
                        for m in self.graph.modules.values()
                        if m.condon_status == ModuleStatus.CONDON_INCOMPATIBLE
                    ]
                ),
            },
            ...
            "modules": {
                name: {
                    ...
                    "condon_status": info.condon_status.value,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Condon Dependency Analyzer")
    ...
    # Run analysis
    analyzer = CondonDependencyAnalyzer(args.project_root)
    ...
    print(f"Condon compatible: {summary['condon_compatible']}")
    print(f"Condon optimizable: {summary['condon_optimizable']}")
    print(f"Condon limited: {summary['condon_limited']}")
    print(f"Condon incompatible: {summary['condon_incompatible']}")
